function [x_vec,s_gain,signalgain] = PIA_Gain_4wave(wl, pump1, pump2, pl1, pl2, tpa)
format long
simm=1;
t0=clock;
...
% Pump 1 is in TE mode (slow axis)
pmp_n_1 = N_te;
pmp_gvd_1 = gvd_te;
pmp_b1_1 = b_1_te_l;
pmp_b2_1 = b_2_te_l;
...
% Pump 2 is in TM mode (fast axis)
pmp_n_2 = N_tm;
pmp_gvd_2 = gvd_tm;
pmp_b1_2 = b_1_tm_l;
pmp_b2_2 = b_2_tm_l;
...
sig_n = N_te;      % make all in TM mode
idl_n = N_tm;
sig_gvd = gvd_te;
idl_gvd = gvd_tm;
sig_b1 = b_1_te_l;
...
lambda_wave3= pl1*1e-9; % 3rd wave wavelength (pump)
lambda_wave4 = pl2*1e-9;
lambda_wave2 = 1/(1/lambda_wave3+1/lambda_wave4-1/lambda_wave1); % 2nd wavelength, from energy conservation, (idler)
...
k0_4=2*pi/lambda_wave4;        % free-space propagation constant 4th wave
...
n_wave3_d = double(pmp_n_1(lambda_wave3));
n_wave4_d = double(pmp_n_2(lambda_wave4));
...
k_wave4_d=2*pi*n_wave4_d/lambda_wave4; 
...
vg_wave3_d = double(pmp_vg_1(lambda_wave3));
vg_wave4_d = double(pmp_vg_2(lambda_wave4));
...
beta_wave3_o = double(pmp_gvd_1(lambda_wave3));
beta_wave4_o = double(pmp_gvd_2(lambda_wave4));
...
gvd_wave4_o=1.0*(beta_wave4_o/2)*(omega.^2); 
...
gvm_wave4_d= 1.0*N_wave4_d*(omega); 
...
Tr_wave4(1:num_period*2*iterations+2) = 1;
...
gvd_wave4_m(1:zStepL-1,length(gvd_wave4_d)) = [0];
...
for zstep = 1:zStepL-1
    ...
    gvm_wave4_m(zstep,:) = gvm_wave4_o;

    gvd_wave1_m(zstep,:) = gvd_wave1_o;
    gvd_wave2_m(zstep,:) = gvd_wave2_o;
    gvd_wave3_m(zstep,:) = gvd_wave3_o; 
    gvd_wave4_m(zstep,:) = gvd_wave4_o;
    
    alpha_wave4_m(zstep) = alpha_wave4_o;
...
Structure = {length_m deltaz_m n_wave1_m n_wave2_m n_wave3_m n_wave4_m deltak_m chi2_m gvm_wave1_m gvm_wave2_m gvm_wave3_m gvm_wave4_m gvd_wave1_m gvd_wave2_m gvd_wave3_m gvd_wave4_m alpha_wave4_m alpha_wave3_m alpha_wave2_m alpha_wave1_m A_eff_m n2_wave4_m n2_wave3_m n2_wave2_m n2_wave1_m a2_wave1_m a2_wave2_m a2_wave3_m a2_wave4_m A_eff_3_m};
...
amp_wave4=sqrt(amp_wave4_sqar);   % input 3rd wave amplitude
...
e_wave4=amp_wave4*sech(-(t/(sqrt(1)*T0_wave4)).^1);   % ultrafast pulse in sech form
...
i_fftnorm_wave4=[];   % 3rd wave normalized power spectrum
...
%% Memory Pre-Allocation
      ...
      power_wave4(1:numplots+1)=[deltat*sum((abs(e_wave4).^2))] ;
      i_wave4(1:length(t),1:numplots+1)=[0];
      i_wave4(1:length(t),1)=[abs(e_wave4').^2];
      inorm_wave4(1:length(t),1:numplots+1)=[0];
      inorm_wave4(1:length(t),1)=[(abs(e_wave4').^2)/max(abs(e_wave4').^2)];
      e_wave4_field(1:length(t),1:numplots+1)=[0];
      e_wave4_field(1:length(t),1)=[e_wave4'];
      i_fft_wave4(1:length(t),1:numplots+1)=[0];
      i_fft_wave4(1:length(t),1)=[abs(deltat*fftshift(fft(e_wave4'))).^2 ];
      i_fftnorm_wave4(1:length(t),1:numplots+1)=[0];
      i_fftnorm_wave4(1:length(t),1)=[abs(fftshift(fft(e_wave4'))).^2/max(abs(fftshift(fft(e_wave4'))).^2) ];
%% MAIN PROGRAM
z_count = 0;
...
n_wave4 = Structure{6}(Material_step - 1);
deltak = Structure{7}(Material_step - 1);
chi2 = Structure{8}(Material_step - 1);
gvm_wave1 = Structure{9}((Material_step - 1),:);
gvm_wave2 = Structure{10}((Material_step - 1),:);
gvm_wave3 = Structure{11}((Material_step - 1),:);
gvm_wave4 = Structure{12}((Material_step - 1),:);
gvd_wave1 = Structure{13}((Material_step - 1),:);
gvd_wave2 = Structure{14}((Material_step - 1),:);
gvd_wave3 = Structure{15}((Material_step - 1),:);
gvd_wave4 = Structure{16}((Material_step - 1),:);
alpha_wave4 = Structure{17}(Material_step - 1);
alpha_wave3 = Structure{18}(Material_step - 1);
alpha_wave2 = Structure{19}(Material_step - 1);
alpha_wave1 = Structure{20}(Material_step - 1);
A_eff_2nd = Structure{21}(Material_step - 1);
n2_wave4 = Structure{22}(Material_step - 1);
n2_wave3 = Structure{23}(Material_step - 1);
n2_wave2 = Structure{24}(Material_step - 1);
n2_wave1 = Structure{25}(Material_step - 1);
a2_wave1 = Structure{26}(Material_step - 1);
a2_wave2 = Structure{27}(Material_step - 1);
a2_wave3 = Structure{28}(Material_step - 1);
...
c_chi3_wave4 = 2*j*(n2_wave4*2*pi)/(A_eff_3rd*lambda_wave4);
...
e_wave4 = e_wave4*Tr_wave4(loop_step);
...
% Saving the results
   if rem(loop_step,njump)==0  % it saves the result (only if loop_step is multiple of njump)
      ...
      power_wave4(z_count+1)=[deltat*sum((abs(e_wave4).^2))] ;
      i_wave4(1:length(t),z_count+1)=[abs(e_wave4').^2];
      inorm_wave4(1:length(t),z_count+1)=[(abs(e_wave4').^2)/max(abs(e_wave4').^2)];
      e_wave4_field(1:length(t),z_count+1)=[e_wave4'];
      i_fft_wave4(1:length(t),z_count+1)=[abs(es_wave4').^2];
      i_fftnorm_wave4(1:length(t),z_count+1)=[abs(es_wave4').^2/max(abs(es_wave4').^2)];
      %
...
power_wave4_L(gradual)=(1e+3)*power_wave4(1)/(2*tmax);

eta_L(gradual)=100*power_wave2_L(gradual)*(1e-3)/(power_wave3_L(gradual)*power_wave1_L(gradual)*power_wave4_L(gradual));
end
%% signal gain
        x = 1e3*L*x;
        x_vec = x;
